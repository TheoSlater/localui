...
export function ModelSelector({ providers, activeModel, onSelect }: ModelSelectorProps) {
  ...
  const rows = useMemo(
    () =>
      Array.from(grouped.entries()).flatMap(([providerName, items]) => [
        { type: 'provider' as const, key: providerName, providerName: items[0].providerName },
        ...items.map((model) => ({
          type: 'model' as const,
  ...
  const setAsDefault = () => {
    ...
    setSettings({ defaultModel: selectedModel });
    const provider = providers.find((item) => item.id === selectedModel.providerId);
    toast.add({
      title: 'Default model set',
      description: `${provider?.name ?? 'Provider'} · ${selectedModel.modelId}`,
    });
  };
  ...
  return (
    <div className="flex flex-col items-start">
      ...
      <button
        ...
        className="text-muted-foreground hover:text-foreground px-2.5 text-[10px] leading-4 transition-colors duration-(--motion-duration-quick)"
      >
        Set as default
      </button>
    </div>
